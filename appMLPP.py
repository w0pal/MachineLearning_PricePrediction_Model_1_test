...
st.title("📈 Prediksi Harga Penutupan Aset - Streamlit App")

with st.sidebar:
    ticker = st.text_input("Masukkan Ticker (contoh: AAPL, BTC-USD, etc)", value="AAPL")

if ticker:
    st.subheader(f"📊 Data dan Prediksi untuk {ticker}")
    ...
    fig.add_trace(go.Scatter(x=[df.index[-1] + timedelta(days=1)], y=[pred_price], name='Prediksi',
                             mode='markers+text', text=[f"${pred_price:.2f}"],
